...
class Link:
    def extract_links(self, res):
        links = [
            ...
            URL(
                "https://www.netvendeur.com/prix/region-provence-alpes-cote-d-azur-18/"
            ),
        ]
        return links
...
class NetvendeurSpider(CrawlSpider):
    name = "netvendeur"
    start_urls = ["https://www.netvendeur.com/prix-immobilier/"]
    res = {
        "regionid": None,
        "departmentid": None,
        "cityid": None,
    ...
    rules = (
        Rule(Link(), callback="parse_region", cb_kwargs=dict(parenturl=start_urls[0])),
    )
    result = []
    custom_settings = {
    ...
    def from_crawler(cls, crawler, *args, **kwargs):
        ...
        crawler.signals.connect(
            spider.after_spider_closed, signal=signals.spider_closed
        )
        return spider
    ...
    def parse_item(self, response, **__):
        ...
        breadcrumbs = response.css(
            '[itemprop="itemListElement"] [itemprop="name"]::text'
        ).extract()[2:]
        breadcrumbs_dict = dict(
            zip_longest(["region", "dept", "city", "quarter", "proximité"], breadcrumbs)
        )
        bas_maison, moyen_maison, haut_maison = (
            response.xpath('normalize-space(//p[@class="re-prices"])')
            .extract_first()
            .split("<")
        )
        ...
        table_evo = [
            [td_or_th.css("::text").extract_first() for td_or_th in tr.xpath("./*")]
        ...
        price_chart = (
            response.css('script:contains("ct-chart-price")').re_first(
                "data:\s*({[\w\W]*?}),\s*options"
            )
            or "{}"
        )
        price_chart = json.loads(
            re.sub("(\w+):", r'"\1":', price_chart).replace("'", '"')
        )

        price_chart = {
            f'{data_set["label"]} {label}': value
            for data_set in price_chart["datasets"]
            for label, value in zip(price_chart["labels"], data_set["data"])
        }
        ...
        result = {
            ...
            "Prix bas apparetement": (re.findall("^([^€]*)", appt_bas_maison) or [""])[
                0
            ].replace(" ", ""),
            "Prix moyen apparetement": (
                re.findall("^([^€]*)", appt_moyen_maison) or [""]
            )[0].replace(" ", ""),
            "Prix haut apparetement": (
                re.findall("^([^€]*)", appt_haut_maison) or [""]
            )[0].replace(" ", ""),
            **{
            ...
            **self._partition_by_number_of_pieces(response),
            ...
            "Répartition selon l'activité (Hommes)": self._get_activity_by_men(
                response
            ),
            "Répartition selon l'activité (Femmes)": self._get_activity_by_women(
                response
            ),
    ...
    def parse_region(self, response, **kwargs):
        ...
        for link in LinkExtractor(
            restrict_xpaths=[
                '//h2[contains(., "départements")]/../div[contains(@class, "list_dep")]'
            ],
            allow=["/prix/"],
    ...
    def parse_dept(self, response, **kwargs):
        ...
        if response.css("#prix-departement .black_title .black_title a"):
            ...
            for link in LinkExtractor(
                restrict_xpaths=[
                    '//h3[contains(., "villes")]/../div[contains(@class, "list_dep")]',
                    '//*[@id="prix-arrondissement"]',
                ],
                allow=["/prix/"],
            ).extract_links(response):
                yield response.follow(
    ...
    def expand_quater(self, response, **kwargs):
        for link in LinkExtractor(
            ...
            yield response.follow(
                ...
                cb_kwargs=dict(
                    typeofdata="city_streets", parenturl=kwargs["parenturl"]
                ),
            )

    def parse_city(self, response, **kwargs):
        yield self.parse_item(response, typeofdata="city", **kwargs)
        if response.css("#prix-rues .red"):
            for link in LinkExtractor(
                restrict_css=["#prix-rues .red"], allow=["/prix/"]
            ).extract_links(response):
                yield response.follow(
                    link.url,
            ...
            if response.css("div#prix-autre-quartier"):
                for link in LinkExtractor(
                    restrict_css=["div#prix-autre-quartier"], allow=["/prix/"]
                ).extract_links(response):
                    yield response.follow(
                        link.url,
                        callback=self.parse_item,
    ...
    def parse_quarter(self, response, **kwargs):
        ...
        for link in LinkExtractor(
            restrict_css=["div#prix-rue"], allow=["/prix"]
        ).extract_links(response):
    ...
    def _partition_by_area(self, response):
        return self._get_maison_and_appartments(
            response, "Répartition selon la superficie"
        )

    def _partition_by_number_of_pieces(self, response):
        return self._get_maison_and_appartments(
            response, "Répartition selon le nombre de pièce"
        )
    ...
    def _get_maison_and_appartments(self, response, _key):
        ...
        values = [
            {
                "maison": report.xpath('.//span[@class="grap-maison"]/@style').re_first(
                    "[\d.]+"
                )
                or "0",
                "appartment": report.xpath(
                    './/span[@class="grap-val"]/@style'
                ).re_first("[\d.]+")
                or "0",
            }
        ...
        return {
            head: json.dumps(value, ensure_ascii=False).replace('"', "'")
    ...
    def _get_volume_table(self, response):
        ...
        head = [
            h for h in table.css("tr")[0].xpath("./td/text()").extract() if h.strip()
        ]
    ...
    def _get_evolution_table(self, response):
        ...
        for v in response.css(".verticale_volume"):
            ...
            result[f"maison, {head}"] = v.css(".maisonv div::attr(style)").re_first(
                "[\d.]+"
            )
            result[f"appartment, {head}"] = v.css(
                ".appartementv div::attr(style)"
            ).re_first("[\d.]+")
    ...
    def _get_chart_data(self, response, _key):
        ...
        if raw_json:
            return json.dumps(
                dict(zip(raw_json["labels"], raw_json["datasets"][0]["data"])),
                ensure_ascii=False,
            ).replace('"', "'")
    ...
    def _price_and_estimate_tables(self, response):
        info_table = [
            t
            for t in response.css("table:contains(Habitants) td::text").extract()
            if t.strip()
        ]
